...
fn main() {
    ...
    app.run(|app_state: &mut State, window: &mut Window| {
        ...
        if let Some(last_vector) = state.user_input.last() {
            if let Some((num, key)) = last_vector.last() {
        ...
        for key_down in app_state.keyboard().get_keys_down() {
            match key_down {
                Key::A => {
                    ...
                    state.user_input.push(vec![(app_state.step() as u16, 0)]);
                },
                Key::D => {
                    state.spaceship_move_x(1);
                    state.user_input.push(vec![(app_state.step() as u16, 1)]);
                },
                _ => (),
            }
        }
...
fn vec_to_hex_string(input: Vec<Vec<(u16, u8)>>) -> String {
    let mut result = String::new();

    for inner_vec in input {
        for (num, boolean) in inner_vec {
            assert!(num <= 4095);
            assert!(boolean <= 1);
...
fn hex_string_to_vec(hex_string: &str) -> Vec<Vec<(u16, u8)>> {
    ...
    for chunk in hex_string.as_bytes().chunks(4) {
        ...
        // Añadir el par (u16, u8) al vector interno
        inner_vec.push((num, boolean));

        // Aquí podrías optar por agregar el inner_vec a result después de cada chunk si tienes varios vectores internos
    }

    // Para este ejemplo, todo se añade en un solo vector
